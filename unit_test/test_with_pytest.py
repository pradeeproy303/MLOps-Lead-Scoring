...
def test_load_data_into_db():
    ...
    loaded_data_test_case = pd.read_sql('select * from loaded_data_test_case', cnx_ut)
    
    cnx.close()
    cnx_ut.close()
    
    assert loaded_data_test_case.equals(loaded_data)
...
def test_map_city_tier():
    ...
    assert city_tier_mapped_test_case.equals(city_tier_mapped_df)
...
def test_map_categorical_vars():
    ...
    SAMPLE USAGE
        output=test_map_categorical_vars()
    ...
    assert categorical_variables_mapped_test_case.equals(categorical_variable_mapped)
...
def test_interactions_mapping():
    ...
    interactions_mapped = interactions_mapped.reindex(sorted(interactions_mapped.columns), axis=1)
    
    cnx_ut = sqlite3.connect(DB_PATH+UNIT_TEST_DB_FILE_NAME)
    interactions_mapped_test_case = pd.read_sql('select * from interactions_mapped_test_case', cnx_ut)
    interactions_mapped_test_case = interactions_mapped_test_case.reindex(sorted(interactions_mapped_test_case.columns), axis=1)

    cnx.close()
    cnx_ut.close()
    
    assert interactions_mapped_test_case.equals(interactions_mapped)
